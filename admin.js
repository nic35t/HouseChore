...
class AdminApp {
    ...
    async load() {
        ...
        if(snap.exists()) {
            this.localRules = snap.data().rules || [];
            this.render();
        }
    }

    render() {
        const cont = document.getElementById('rulesList');
        cont.innerHTML = '';
        this.localRules.forEach((r, i) => {
            const div = document.createElement('div');
            div.className = "bg-white p-6 rounded-2xl border border-slate-100 relative group";
            div.innerHTML = `
                <button onclick="window.admin.delRule(${i})" class="absolute top-4 right-4 text-slate-300 hover:text-red-500 font-bold text-[10px] uppercase">Remove</button>
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <input type="text" placeholder="키워드 (예: 설거지|주방)" value="${r.match}" onchange="window.admin.updRule(${i},'match',this.value)" class="p-3 bg-slate-50 rounded-xl text-sm font-bold focus:outline-orange-500">
                    <input type="number" placeholder="포인트" value="${r.pts}" onchange="window.admin.updRule(${i},'pts',this.value)" class="p-3 bg-slate-50 rounded-xl text-sm font-black text-orange-600 focus:outline-orange-500">
                </div>
                <input type="text" placeholder="구역 (예: 주방)" value="${r.zone}" onchange="window.admin.updRule(${i},'zone',this.value)" class="w-full p-2.5 text-xs bg-slate-50/50 rounded-lg">
            `;
